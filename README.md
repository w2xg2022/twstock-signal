...
- **賣出價（已結束）**：進場後**第 18～20 個交易日的均價（TWAP）**——約一個月跑完一輪。取持有期尾段這 3 天的均價、而非單押第 20 天，是為了用區間均價去除單日雜訊；窗口刻意收在 **第 20 天以內**，讓出場完全落在「持有 20 個交易日」的設計範圍內，不外溢到持有期之外。尚未滿 20 天的週次（「預測中」）則顯示追到最新收盤的**當前價**。
